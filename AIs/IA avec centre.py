...
# %%
def target_center (graph,player_location,piece_of_cheese):
    distances,routing_table=Dijkstra(graph,player_location)
    chemin=find_route(routing_table,player_location,piece_of_cheese)
    return chemin
...
def preprocessing (maze_map, maze_width, maze_height, player_location, opponent_location, pieces_of_cheese, time_allowed) :
    global meta_graph, best_paths,path_to_the_center
    path_to_the_center=target_center (maze_map,player_location,(16,15))
...
def turn (maze_map, maze_width, maze_height, player_location, opponent_location, player_score, opponent_score, pieces_of_cheese, time_allowed) :
    global meta_graph,eaten_pieces,testing,path_to_new_target
    global moving,path_to_the_center,center
    ...
    if not center:
        if len(path_to_the_center)==1:
            center=True
        else:
            return move_from_locations(path_to_the_center)
    else:
        if moving: 
            if (not path_to_new_target) or (test and testing[-1]==eaten_pieces[-1]):
                moving = False
        if not moving:
            new_target,path_to_new_target=find_closest_piece_of_cheese(maze_map,player_location,pieces_of_cheese)
